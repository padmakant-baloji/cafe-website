...
function buildOrderEmailText(body) {
    const { customerName, mobileNumber, customerCity, deliveryAddress, items, subtotal, deliveryFee, discount, couponCode, total } = body;
    let text = `Order from Baloji Cafe\n\n`;
    ...
    if (typeof discount === 'number' && !Number.isNaN(discount) && discount > 0) {
        text += `Discount${couponCode ? ` (${couponCode})` : ''}: -₹${discount}\n`;
    }
...
async function submitOrder(body) {
    ...
    const subtotal =
        typeof body.subtotal === 'number' && !Number.isNaN(body.subtotal)
            ? body.subtotal
            : parseInt(String(body.subtotal ?? 0), 10);
    const deliveryFee =
        typeof body.deliveryFee === 'number' && !Number.isNaN(body.deliveryFee)
            ? body.deliveryFee
            : parseInt(String(body.deliveryFee ?? 0), 10);
    const discount =
        typeof body.discount === 'number' && !Number.isNaN(body.discount)
            ? body.discount
            : parseInt(String(body.discount ?? 0), 10);
    const couponCode = trimStr(body.couponCode, 64);
    ...
    if (Number.isNaN(subtotal) || subtotal !== computedSubtotal) {
        throw httpError(400, 'Subtotal mismatch.');
    }

    const computedPayable = Math.max(0, subtotal - Math.max(0, discount)) + Math.max(0, deliveryFee);
    if (Number.isNaN(total) || total !== computedPayable) {
