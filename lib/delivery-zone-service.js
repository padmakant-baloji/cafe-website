 ...
 * Pass venueId = null to get the main-venue (Baloji Cafe) zones.
 */
...
async function getDeliveryConfig(venueId, cityName, subtotal) {
    ...
    if (!zone) {
        const { getVenueById, resolvePublicVenue } = require('./venue-service');
        const venue = venueId != null ? await getVenueById(venueId) : await resolvePublicVenue();
        const cityLower = String(cityName || '').trim().toLowerCase();
        
        if (venue && !venue.isDefault) {
            const { rows } = await query('SELECT id FROM delivery_zones WHERE venue_id = $1 LIMIT 1', [venue.id]);
            if (rows.length === 0) {
                const vCityLower = String(venue.city || '').trim().toLowerCase();
                if (vCityLower && vCityLower === cityLower) {
                    return { deliveryFee: 8, minOrder: 0, allowed: true, zoneName: vCityLower };
                }
            }
            return { deliveryFee: 0, minOrder: 0, allowed: false, zoneName: '' };
        }
        
        // No zone configured for main venue — allow order, default ₹8 fee to match frontend fallback
        return { deliveryFee: 8, minOrder: 0, allowed: true, zoneName: '' };
    }
...
/**
 * Get all zones across all venues, formatted as a map of venueId -> array of zones.
 * Main venue is keyed as 'default'.
 */
async function getAllDeliveryZonesForClient() {
    const { rows } = await query(
        `SELECT venue_id, city, min_order, delivery_fee, free_delivery_above
         FROM delivery_zones
         WHERE enabled = TRUE
         ORDER BY venue_id ASC, city ASC`
    );
    
    const map = { 'default': [] };
    for (const r of rows) {
        const vKey = r.venue_id == null ? 'default' : String(r.venue_id);
