    ...
    for (const cat of categories || []) {
        ...
        if (Array.isArray(cat.subsections) && cat.subsections.length) {
            next.subsections = cat.subsections.map((sub) => ({
                ...
                items: (sub.items || [])
                    .filter(itemIsListed)
                    .map((item) => annotateItemForCustomer(item, venue, options))
            }));
        } else {
...
function flattenMenuItems(menuPayload) {
    ...
    for (const cat of menuPayload.categories || []) {
        const pushItem = (item) => {
            ...
            const key = `${item.venueId}:${item.id || item.name}:${item.customerPrice ?? item.price}`;
            map.set(key, item);
            map.set(`${item.venueId}:${item.name}:${item.customerPrice ?? item.price}`, item);
        };
...
async function validateAndPriceCustomerOrder(body, customerCity) {
    ...
    if (subtotal > 0) {
        if (isPartnerOrder) {
            deliveryFee = PARTNER_DELIVERY_FEE_RS;
        } else {
            deliveryFee = cityLower === 'kudachi' ? KUDACHI_DELIVERY_FEE : NON_KUDACHI_DELIVERY_FEE;
        }
    }

    if (!isPartnerOrder && cityLower && cityLower !== 'kudachi' && subtotal < MIN_NON_KUDACHI_ORDER_VALUE) {
        throw Object.assign(
            new Error(`Minimum order for delivery outside Kudachi is ₹${MIN_NON_KUDACHI_ORDER_VALUE}.`),
            { statusCode: 400 }
        );
    }
