...
const { getDeliveryConfig, getDeliveryZonesForClient } = require('./delivery-zone-service');
...
async function getAggregatedCustomerMenu() {
    ...
    // Fetch delivery zones for the main venue (Baloji Cafe) so clients can replicate fee calc.
    const mainDeliveryZones = await getDeliveryZonesForClient(null);
    ...
    return {
        ...
        deliveryZones: mainDeliveryZones,
        venues,
        categories
    };
}
...
async function validateAndPriceCustomerOrder(body, customerCity) {
    ...
    for (const row of items) {
        ...
        const price = parseInt(String(catalogItem.customerPrice ?? catalogItem.price), 10);
    ...
    // Look up delivery fee from DB-configured delivery zones.
    // For main venue (Baloji), venueId = null; for partners, use their venue id.
    const zoneVenueId = isPartnerOrder ? orderVenueId : null;
    const deliveryConfig = await getDeliveryConfig(zoneVenueId, cityLower || 'kudachi', subtotal);
    let deliveryFee = subtotal > 0 ? deliveryConfig.deliveryFee : 0;

    // If the zone has a free_delivery_above threshold and subtotal meets it,
    // deliveryConfig already set deliveryFee to 0. But we also need a reduced fee.
    // The _default zone seeds: delivery_fee=40, free_delivery_above=500.
    // When subtotal >= 500, deliveryFee becomes 0. User wants ₹8 instead.
    // To handle the "reduced fee" case: if free_delivery_above is set and subtotal >= it,
    // we fall back to the 'kudachi' zone fee (₹8). This is handled by checking for a
    // second zone. For simplicity, we use the free_delivery_above to zero-out the fee,
    // and apply a flat base fee (from kudachi zone) when the threshold is met.
    if (!isPartnerOrder && subtotal > 0 && deliveryConfig.deliveryFee === 0 && deliveryConfig.zoneName === '_default') {
        // Threshold was met — apply the local (Kudachi) delivery fee instead of free.
        const localConfig = await getDeliveryConfig(null, 'kudachi', subtotal);
        deliveryFee = localConfig.deliveryFee;
    }

    if (!isPartnerOrder && cityLower && deliveryConfig.minOrder > 0 && subtotal > 0 && subtotal < deliveryConfig.minOrder) {
        throw Object.assign(
