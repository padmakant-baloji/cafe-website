...
function clearBalojiCafeMenuCache() {
    // No longer applicable as main venue has no menu.
}
...
function annotateItemForCustomer(item, venue, options = {}) {
    ...
    const rawSizes = Array.isArray(item.sizes) ? item.sizes : (item.sizes && typeof item.sizes === 'object' ? Object.entries(item.sizes).map(([k, v]) => ({ ...v, label: k })) : []);
    if (rawSizes.length) {
        next.sizes = rawSizes.map((size) => {
            const basePrice = parseInt(String(size.onlinePrice ?? size.price), 10) || 0;
            const customerPrice = Math.round(basePrice * (1 + markup / 100));
            return {
                ...size,
                basePrice,
                price: customerPrice,
    ...
    const basePrice = parseInt(String(item.onlinePrice ?? item.price), 10) || 0;
    const customerPrice = Math.round(basePrice * (1 + markup / 100));
...
async function getAggregatedCustomerMenu() {
    ...
    const { rows } = await query(
        `SELECT v.id, v.slug, v.name, v.city, v.contact_mobile, v.hours_text, v.menu_catalog, ss.accepting_orders, v.partner_markup_pct
         FROM venues v
    ...
    for (const row of rows) {
        ...
        categories.push(
            ...annotateCategoryTree(catalog.categories || catalog || [], venue, {
                markup: Number(row.partner_markup_pct) || 0
            })
        );
    }
    ...
    const defVenue = venues.length > 0 ? venues[0] : { id: null, slug: null, name: null };

    return {
        defaultVenueId: defVenue.id,
        defaultVenueSlug: defVenue.slug,
        defaultVenueName: defVenue.name,
...
async function saveAdminMenuForVenue(actingVenue, targetVenueId, body) {
    ...
    if (isMainVenue(venue)) {
        throw Object.assign(new Error('Super Admin venue cannot have a menu.'), { statusCode: 400 });
    }
...
function flattenMenuItems(menuPayload) {
    ...
    for (const cat of menuPayload.categories || []) {
        const pushItem = (item) => {
            ...
            const isMainVenue = item.isMainVenue !== false;
            
            const basePrice = parseInt(String(item.customerPrice ?? item.onlinePrice ?? item.price), 10);
            if (Number.isFinite(basePrice)) {
                const expectedPrice = isMainVenue ? Math.round(basePrice * 0.8) : basePrice;
                map.set(`${item.venueId}:${itemId}:${expectedPrice}`, item);
                map.set(`${item.venueId}:${item.name}:${expectedPrice}`, item);
            }

            const rawSizes = Array.isArray(item.sizes) ? item.sizes : (item.sizes && typeof item.sizes === 'object' ? Object.entries(item.sizes).map(([k, v]) => ({ ...v, label: k })) : []);
            for (const size of rawSizes) {
                const sizePrice = parseInt(String(size.customerPrice ?? size.onlinePrice ?? size.price), 10);
                ...
                const expectedSizePrice = isMainVenue ? Math.round(sizePrice * 0.8) : sizePrice;
                map.set(`${item.venueId}:${itemId}:${expectedSizePrice}`, sizedItem);
                map.set(`${item.venueId}:${sizeName}:${expectedSizePrice}`, sizedItem);
            }
        };
...
async function validateAndPriceCustomerOrder(body, customerCity) {
    ...
    for (const row of items) {
        ...
        if (!catalogItem) {
            throw Object.assign(new Error(`Price mismatch or item not available: ${String(row.name || '').trim()}. Please empty your cart and add the item again.`), {
                statusCode: 400
            });
        }
        ...
        if (catalogItem.isMainVenue !== false) { // Default to true if missing for safety
            price = Math.round(originalPrice * (1 - DISCOUNT_PERCENT / 100));
        }
    ...
    let cityLower = String(customerCity || '').trim().toLowerCase();
    if (!cityLower) {
        cityLower = 'kudachi';
    }
    ...
    if (!isPartnerOrder && cityLower) {
        deliveryConfig = await getDeliveryConfig(mainVenue.id, cityLower, subtotal);
        if (deliveryConfig && !deliveryConfig.allowed) {
            throw Object.assign(new Error(`Delivery is not available in ${customerCity}.`), { statusCode: 400 });
        }
        deliveryFee = deliveryConfig ? deliveryConfig.deliveryFee : 0;
    } else if (isPartnerOrder) {
        // Flat ₹8 delivery fee for partners
    ...
    if (!isPartnerOrder && cityLower && deliveryConfig && deliveryConfig.minOrder > 0 && subtotal > 0 && subtotal < deliveryConfig.minOrder) {
        throw Object.assign(
            new Error(`Minimum order for delivery is ₹${deliveryConfig.minOrder}.`),
            { statusCode: 400 }
        );
    }
