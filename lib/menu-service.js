...
let quickkartMenuCache = null;

function loadBalojiCafeMenuJson() {
    if (quickkartMenuCache) return quickkartMenuCache;
    const filePath = path.join(__dirname, '..', 'menu.json');
    const raw = fs.readFileSync(filePath, 'utf8');
    quickkartMenuCache = JSON.parse(raw);
    return quickkartMenuCache;
}

function clearBalojiCafeMenuCache() {
    quickkartMenuCache = null;
}
...
async function getAggregatedCustomerMenu() {
    ...
    const quickkartMenu = loadBalojiCafeMenuJson();
    const categories = annotateCategoryTree(quickkartMenu.categories || [], mainVenue, {
        markup: 0,
        preserveSource: true
    });
...
async function getAdminMenuForVenue(actingVenue, targetVenueId = null) {
    ...
    if (isMainVenue(venue)) {
        ...
        return {
            ...
            source: 'menu.json',
            editable: false,
            categories: quickkartMenu.categories || []
        };
    }
...
async function saveAdminMenuForVenue(actingVenue, targetVenueId, body) {
    ...
    if (isMainVenue(venue)) {
        throw Object.assign(new Error('Baloji Cafe Cafe menu is managed in menu.json.'), { statusCode: 400 });
    }
