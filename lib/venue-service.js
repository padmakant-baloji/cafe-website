...
const ADMIN_USER = process.env.ADMIN_USER || 'quickkartcafe';
const ADMIN_PASS = process.env.ADMIN_PASS || 'admin';
const DEFAULT_VENUE_SLUG = 'quickkartcafe';
...
async function getDefaultVenue() {
    const { rows } = await query(
        `SELECT id, slug, name, city, address_line, contact_mobile, hours_text, table_count, parcel_count, walkin_mobile, is_default, admin_user, venue_type, payment_qr_code
         FROM venues
...
async function setFloorConfig(venueId, input) {
    ...
    const { rows } = await query(
        `UPDATE venues
         ...
         WHERE id = $1
         RETURNING id, slug, name, city, address_line, contact_mobile, hours_text, table_count, parcel_count, walkin_mobile, is_default, admin_user, venue_type, payment_qr_code`,
