...
async function getDefaultVenue() {
    const { rows } = await query(
        `SELECT id, slug, name, city, address_line, contact_mobile, hours_text, table_count, parcel_count, walkin_mobile, is_default, admin_user, venue_type
         FROM venues
...
async function createVenueByMain(actingVenue, input) {
    ...
    const { rows } = await query(
        `INSERT INTO venues (slug, name, city, admin_user, admin_pass, table_count, parcel_count, walkin_mobile, contact_mobile, hours_text, venue_type, is_default)
         ...
         RETURNING id, slug, name, city, address_line, contact_mobile, hours_text, table_count, parcel_count, walkin_mobile, is_default, admin_user, venue_type`,
