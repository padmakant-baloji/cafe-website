...
async function createOrder(customerMobile, items, totals, deliveryAddress = null, venueId = 1, channel = 'delivery') {
    ...
    try {
        const { rows } = await query(
            `INSERT INTO orders (customer_id, customer_mobile, status, items, subtotal, delivery_fee, discount, coupon_code, total, delivery_address, venue_id, channel)
             SELECT c.id, c.mobile, 'pending', $2::jsonb, $3, $4, $5, $6, $7, $8::jsonb, $9, $10
             FROM customers c
             WHERE c.mobile = $1
        ...
        const { rows } = await query(
            `INSERT INTO orders (customer_mobile, status, items, subtotal, delivery_fee, discount, coupon_code, total, delivery_address, venue_id, channel)
             VALUES ($1, 'pending', $2::jsonb, $3, $4, $5, $6, $7, $8::jsonb, $9, $10)
             RETURNING id, customer_mobile, status, items, subtotal, delivery_fee, discount, coupon_code, total, delivery_address, created_at, updated_at, venue_id, channel`,
            payload
        );
        const order = rows[0];
...
async function listOrdersForCustomer(customerMobile, limit = 50) {
    const { rows } = await query(
        `SELECT o.id, o.status, o.payment_status, o.items, o.total, o.delivery_address, o.created_at, o.updated_at,
...
async function listAllOrdersForAdmin(venueId, limit = 100) {
    const vid = parseInt(String(venueId), 10) || 1;
    const { rows } = await query(
        `SELECT o.id, o.status, o.payment_status, o.items, o.subtotal, o.delivery_fee, o.discount, o.coupon_code, o.total,
...
async function getOrderById(orderId) {
    const { rows } = await query(
        `SELECT o.id, o.customer_mobile, o.status, o.payment_status, o.items, o.subtotal, o.delivery_fee, o.discount, o.coupon_code, o.total,
    ...
    if (paymentMethod) {
        ...
        const { rows } = await query(
            `UPDATE orders SET status = $2, payment_status = $4, order_meta = $3::jsonb, updated_at = NOW() WHERE id = $1
             RETURNING id, customer_mobile, status, payment_status, items, subtotal, delivery_fee, discount, coupon_code, total, delivery_address, created_at, updated_at, channel, order_meta`,
