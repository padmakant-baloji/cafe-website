 ...
 * @param {number} total
 */
async function createOrder(customerMobile, items, totals, deliveryAddress = null, venueId = 1, channel = 'delivery') {
    const mobile = normalizeMobile(customerMobile);
    const vid = parseInt(String(venueId), 10) || 1;
    const orderChannel = String(channel || 'delivery').trim().toLowerCase() || 'delivery';
    ...
    try {
        const { rows } = await query(
            `INSERT INTO orders (customer_id, customer_mobile, status, items, subtotal, delivery_fee, discount, coupon_code, total, delivery_address, venue_id, channel, daily_order_number)
             SELECT c.id, c.mobile, 'pending', $2::jsonb, $3, $4, $5, $6, $7, $8::jsonb, $9, $10, COALESCE((SELECT MAX(daily_order_number) FROM orders WHERE venue_id = $9 AND created_at AT TIME ZONE 'Asia/Kolkata' >= CURRENT_DATE AT TIME ZONE 'Asia/Kolkata'), 0) + 1
             FROM customers c
             WHERE c.mobile = $1
        ...
        const { rows } = await query(
            `INSERT INTO orders (customer_mobile, status, items, subtotal, delivery_fee, discount, coupon_code, total, delivery_address, venue_id, channel, daily_order_number)
             VALUES ($1, 'pending', $2::jsonb, $3, $4, $5, $6, $7, $8::jsonb, $9, $10, COALESCE((SELECT MAX(daily_order_number) FROM orders WHERE venue_id = $9 AND created_at AT TIME ZONE 'Asia/Kolkata' >= CURRENT_DATE AT TIME ZONE 'Asia/Kolkata'), 0) + 1)
             RETURNING id, daily_order_number, customer_mobile, status, items, subtotal, delivery_fee, discount, coupon_code, total, delivery_address, created_at, updated_at, venue_id, channel`,
            payload
        );
        const order = rows[0];
...
async function listOrdersForCustomer(customerMobile, limit = 50) {
    const { rows } = await query(
        `SELECT o.id, o.daily_order_number, o.status, o.payment_status, o.items, o.total, o.delivery_address, o.created_at, o.updated_at,
                v.name AS venue_name, v.contact_mobile AS venue_contact_mobile, v.payment_qr_code AS venue_payment_qr_code
         FROM orders o
...
async function listAllOrdersForAdmin(venueId, limit = 100) {
    const vid = parseInt(String(venueId), 10) || 1;
    const { rows } = await query(
        `SELECT o.id, o.daily_order_number, o.status, o.payment_status, o.items, o.subtotal, o.delivery_fee, o.discount, o.coupon_code, o.total,
                o.delivery_address, o.created_at, o.updated_at, o.channel, o.order_meta, o.venue_id, o.payment_screenshot,
...
async function getOrderById(orderId) {
    const { rows } = await query(
        `SELECT o.id, o.daily_order_number, o.customer_mobile, o.status, o.payment_status, o.items, o.subtotal, o.delivery_fee, o.discount, o.coupon_code, o.total,
...
async function applyAdminOrderAction(orderId, action, body = {}, venueId = null) {
    ...
    if (paymentMethod) {
        ...
        const { rows } = await query(
            `UPDATE orders SET status = $2, payment_status = $4, order_meta = $3::jsonb, updated_at = NOW() WHERE id = $1
             RETURNING id, daily_order_number, customer_mobile, status, payment_status, items, subtotal, delivery_fee, discount, coupon_code, total, delivery_address, created_at, updated_at, channel, order_meta`,
            [orderId, next, metaJson, paymentStatus]
        );
        const updated = rows[0];
        adminOrderEvents.emit('update', { orderId: updated.id, status: updated.status, payment_status: updated.payment_status });
        return updated;
    }
...
async function cancelOrderByCustomer(orderId, customerMobile) {
    ...
    const { rows } = await query(
        `UPDATE orders SET status = 'cancelled', updated_at = NOW()
         ...
         RETURNING id, daily_order_number, customer_mobile, status, items, total, delivery_address, created_at, updated_at`,
        [id, m]
    );
...
async function createFloorSession(venueId, channel, slotNum, guestLabel = '') {
    ...
    const { rows } = await query(
        `INSERT INTO orders (
            customer_mobile, status, items, subtotal, delivery_fee, discount, coupon_code, total, delivery_address, channel, order_meta, venue_id, daily_order_number
        )
        VALUES ($1, 'accepted', '[]'::jsonb, 0, 0, 0, NULL, 0, NULL, $2, $3::jsonb, $4, COALESCE((SELECT MAX(daily_order_number) FROM orders WHERE venue_id = $4 AND created_at AT TIME ZONE 'Asia/Kolkata' >= CURRENT_DATE AT TIME ZONE 'Asia/Kolkata'), 0) + 1)
        RETURNING id, daily_order_number, customer_mobile, status, items, subtotal, delivery_fee, discount, coupon_code, total, delivery_address, created_at, updated_at, channel, order_meta, venue_id`,
    ...
    if (action === 'floor_void' || action === 'floor_cancel') {
        ...
        const { rows } = await query(
            `UPDATE orders SET status = 'rejected', updated_at = NOW() WHERE id = $1
             RETURNING id, daily_order_number, customer_mobile, status, items, subtotal, delivery_fee, discount, coupon_code, total, delivery_address, created_at, updated_at, channel, order_meta`,
            [id]
        );
        const u = rows[0];
...
async function commitFloorOrderToDb(venueId, body) {
    ...
    const { rows } = await query(
        ...
        VALUES ($1, 'completed', $2::jsonb, $3, 0, 0, NULL, $4, NULL, $5, $6::jsonb, $7, COALESCE((SELECT MAX(daily_order_number) FROM orders WHERE venue_id = $7 AND created_at AT TIME ZONE 'Asia/Kolkata' >= CURRENT_DATE AT TIME ZONE 'Asia/Kolkata'), 0) + 1)
        RETURNING id, daily_order_number, customer_mobile, status, items, subtotal, delivery_fee, discount, coupon_code, total, delivery_address, created_at, updated_at, channel, order_meta, venue_id`,
...
async function cleanupOldScreenshots() {
    try {
        ...
        console.error('cleanupOldScreenshots failed:', err.message);
    }
}

/**
 * Aggregated orders summary across ALL venues for super admin dashboard.
 * Returns per-venue totals (revenue, order count) and a global summary.
 * @param {'today'|'week'|'month'|'all'} period
 */
async function listAllOrdersSummaryForSuperAdmin(period = 'today') {
    let dateFilter = '';
    if (period === 'today') {
        dateFilter = `AND o.created_at AT TIME ZONE 'Asia/Kolkata' >= CURRENT_DATE AT TIME ZONE 'Asia/Kolkata'`;
    } else if (period === 'week') {
    ...
    // Per-venue breakdown for completed orders
    const { rows: venueRows } = await query(`
        SELECT
            ...
            v.venue_type,
            v.is_default,
            v.partner_markup_pct,
            COALESCE(SUM(CASE WHEN o.status = 'completed' THEN o.total ELSE 0 END), 0)::int AS completed_revenue,
            COALESCE(COUNT(CASE WHEN o.status = 'completed' THEN 1 END), 0)::int AS completed_orders,
            COALESCE(COUNT(o.id), 0)::int AS total_orders,
            COALESCE(COUNT(CASE WHEN o.status = 'pending' THEN 1 END), 0)::int AS pending_orders,
            COALESCE(COUNT(CASE WHEN o.status IN ('accepted', 'preparing', 'out_for_delivery') THEN 1 END), 0)::int AS active_orders
        FROM venues v
        LEFT JOIN orders o ON o.venue_id = v.id AND o.customer_mobile <> v.walkin_mobile ${dateFilter}
        GROUP BY v.id, v.name, v.city, v.venue_type, v.is_default, v.partner_markup_pct
    ...
    let foodVenueCount = 0;
    let groceryVenueCount = 0;

    const venues = venueRows.map(r => {
        const revenue = Number(r.completed_revenue) || 0;
        const completedOrders = Number(r.completed_orders) || 0;
        const allOrders = Number(r.total_orders) || 0;
        const pending = Number(r.pending_orders) || 0;
        const active = Number(r.active_orders) || 0;
        const isDefault = r.is_default === true || r.is_default === 't';
        const venueType = r.venue_type === 'grocery' ? 'grocery' : 'food';
        const partnerMarkupPct = Number(r.partner_markup_pct) || 0;
        const venueCommission = Math.round(revenue * (partnerMarkupPct / 100));

        totalRevenue += revenue;
        totalCompletedOrders += completedOrders;
        totalAllOrders += allOrders;
        totalPendingOrders += pending;
        totalActiveOrders += active;
        totalCommission += venueCommission;

        if (!isDefault) {
            if (venueType === 'grocery') groceryVenueCount++;
            else foodVenueCount++;
        }
