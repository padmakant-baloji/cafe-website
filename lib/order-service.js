 ...
 * @param {number} total
 */
async function createOrder(customerMobile, items, total) {
    const mobile = normalizeMobile(customerMobile);
    const payload = [mobile, JSON.stringify(items), total];
    try {
        const { rows } = await query(
            `INSERT INTO orders (customer_mobile, status, items, total)
             VALUES ($1, 'pending', $2::jsonb, $3)
             RETURNING id, customer_mobile, status, items, total, created_at, updated_at`,
            payload
        );
        const order = rows[0];
        ...
        // Legacy-schema fallback: some databases still enforce NOT NULL on orders.customer_id.
        const msg = String(err && err.message ? err.message : '');
        if (err && err.code === '23502' && msg.includes('customer_id')) {
            const { rows } = await query(
                `INSERT INTO orders (customer_id, customer_mobile, status, items, total)
                 SELECT c.id, c.mobile, 'pending', $2::jsonb, $3
                 FROM customers c
                 WHERE c.mobile = $1
                 RETURNING id, customer_mobile, status, items, total, created_at, updated_at`,
                payload
...
async function listOrdersForCustomer(customerMobile, limit = 50) {
    const { rows } = await query(
        `SELECT id, status, items, total, created_at, updated_at
         FROM orders
...
async function listAllOrdersForAdmin(limit = 100) {
    const { rows } = await query(
        `SELECT o.id, o.status, o.items, o.total, o.created_at, o.updated_at,
...
async function getOrderById(orderId) {
    const { rows } = await query(
        `SELECT o.id, o.customer_mobile, o.status, o.items, o.total, o.created_at, o.updated_at,
                c.mobile, c.name, c.city
         FROM orders o
         JOIN customers c ON c.mobile = o.customer_mobile
         WHERE o.id = $1
