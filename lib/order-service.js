 ...
 * @param {number} total
 */
async function createOrder(customerMobile, items, totals, deliveryAddress = null) {
    ...
    try {
        const { rows } = await query(
            `INSERT INTO orders (customer_id, customer_mobile, status, items, subtotal, delivery_fee, discount, coupon_code, total, delivery_address)
             SELECT c.id, c.mobile, 'pending', $2::jsonb, $3, $4, $5, $6, $7, $8::jsonb
             FROM customers c
             WHERE c.mobile = $1
             RETURNING id, customer_mobile, status, items, subtotal, delivery_fee, discount, coupon_code, total, delivery_address, created_at, updated_at`,
            payload
        );
        if (!rows.length) {
        ...
        const { rows } = await query(
            `INSERT INTO orders (customer_mobile, status, items, subtotal, delivery_fee, discount, coupon_code, total, delivery_address)
             VALUES ($1, 'pending', $2::jsonb, $3, $4, $5, $6, $7, $8::jsonb)
             RETURNING id, customer_mobile, status, items, subtotal, delivery_fee, discount, coupon_code, total, delivery_address, created_at, updated_at`,
            payload
        );
        const order = rows[0];
...
async function listAllOrdersForAdmin(limit = 100) {
    const { rows } = await query(
        `SELECT o.id, o.status, o.items, o.subtotal, o.delivery_fee, o.discount, o.coupon_code, o.total,
                o.delivery_address, o.created_at, o.updated_at, o.channel, o.order_meta,
 ...
 * @param {object} [body]
 */
async function applyAdminOrderAction(orderId, action, body = {}) {
...
async function findActiveFloorOrderBySlot(slotKey) {
    const { rows } = await query(
...
async function listFloorSessionsForAdmin() {
    const { rows } = await query(
        `SELECT o.id, o.status, o.items, o.subtotal, o.delivery_fee, o.discount, o.coupon_code, o.total,
                o.delivery_address, o.created_at, o.updated_at, o.channel, o.order_meta,
                c.mobile, c.name, c.city
...
async function createFloorSession(channel, slotNum, guestLabel = '') {
    ...
    if (ch === 'dine_in' && (!Number.isFinite(n) || n < 1 || n > 7)) {
        throw Object.assign(new Error('Table must be between 1 and 7.'), { statusCode: 400 });
    }
    if (ch === 'parcel' && (!Number.isFinite(n) || n < 1 || n > 5)) {
        throw Object.assign(new Error('Parcel slot must be between 1 and 5.'), { statusCode: 400 });
    }
    const slotKey = slotKeyFromChannel(ch, n);
    const busy = await findActiveFloorOrderBySlot(slotKey);
    if (busy) {
    ...
    const { rows } = await query(
        `INSERT INTO orders (
            customer_mobile, status, items, subtotal, delivery_fee, discount, coupon_code, total, delivery_address, channel, order_meta
        )
        VALUES ($1, 'accepted', '[]'::jsonb, 0, 0, 0, NULL, 0, NULL, $2, $3::jsonb)
        RETURNING id, customer_mobile, status, items, subtotal, delivery_fee, discount, coupon_code, total, delivery_address, created_at, updated_at, channel, order_meta`,
        [FLOOR_WALKIN_MOBILE, ch, JSON.stringify(order_meta)]
    );
    const order = rows[0];
    adminOrderEvents.emit('new', { orderId: order.id, customerMobile: FLOOR_WALKIN_MOBILE });
    return order;
}

async function applyFloorOrderAdminPatch(orderId, body) {
    const id = parseInt(String(orderId), 10);
    if (!Number.isFinite(id)) {
        throw Object.assign(new Error('Invalid order id.'), { statusCode: 400 });
    }
 ...
 */
async function commitFloorOrderToDb(body) {
    const src = body && typeof body === 'object' ? body : {};
    const pm = String(src.payment_method || '').trim().toUpperCase();
    if (pm !== 'CASH' && pm !== 'UPI') {
        throw Object.assign(new Error('Choose payment: CASH or UPI.'), { statusCode: 400 });
    }
    ...
    const { rows } = await query(
        ...
        VALUES ($1, 'completed', $2::jsonb, $3, 0, 0, NULL, $4, NULL, $5, $6::jsonb)
        RETURNING id, customer_mobile, status, items, subtotal, delivery_fee, discount, coupon_code, total, delivery_address, created_at, updated_at, channel, order_meta`,
        [FLOOR_WALKIN_MOBILE, JSON.stringify(items), subtotal, total, ch, metaJson]
    );
    const order = rows[0];
