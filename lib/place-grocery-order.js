    ...
    try {
        ...
        const { rows } = await client.query(
            `INSERT INTO orders (customer_mobile, status, items, subtotal, delivery_fee, discount, coupon_code, total, delivery_address, venue_id, channel, daily_order_number)
             VALUES ($1, 'pending', $2::jsonb, $3, $4, $5, $6, $7, $8::jsonb, $9, 'grocery', COALESCE((SELECT MAX(daily_order_number) FROM orders WHERE venue_id = $9 AND created_at AT TIME ZONE 'Asia/Kolkata' >= CURRENT_DATE AT TIME ZONE 'Asia/Kolkata'), 0) + 1)
