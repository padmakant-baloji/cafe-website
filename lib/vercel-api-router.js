...
const { getStoreStatus, setStoreStatus } = require('./store-status');
...
function apiPathname(req) {
    ...
    if (normalized === '/api' && req.query && req.query.path != null) {
        const p = req.query.path;
        const segments = Array.isArray(p)
            ? p
            : String(p)
                  .split('/')
                  .map((s) => s.trim())
        ...
        if (suffix) normalized = `/api/${suffix}`;
    }
    return normalized;
}
...
/**
 * Vercel: one Serverless Function per file (no Next-style catch-alls on static sites).
 * Each `api/**` entry re-exports this handler; routing uses `req.url` pathname.
 */
