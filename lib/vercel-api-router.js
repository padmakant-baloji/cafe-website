...
const { placeGroceryOrderForCustomer } = require('./place-grocery-order');
const {
...
const { signAdminSession } = require('./admin-session');
const {
    getFloorConfig,
    setFloorConfig,
    resolvePublicVenue,
...
const { listDeliveryZones, upsertDeliveryZone, deleteDeliveryZone } = require('./delivery-zone-service');
const {
    normalizeMobile,
...
async function handleGetAllOrdersSummary(req, res) {
    const venue = await requireAdminApi(req, res);
    if (!venue) return;
    try {
        await ensureSchema();
...
async function handleDeleteAdminVenue(req, res, venueId) {
    const venue = await requireAdminApi(req, res);
    if (!venue) return;
    try {
        await ensureSchema();
        const result = await deleteVenueByMain(venue, venueId);
...
async function handleGetGrocery(req, res) {
    try {
        await ensureSchema();
        ...
        return jsonError(res, 500, 'Could not load grocery store.');
    }
}

async function handlePostGroceryOrder(req, res) {
    const session = await resolveCustomerSession(req);
    if (!session) return jsonError(res, 401, 'Sign in required.');
    try {
        await ensureSchema();
        const order = await placeGroceryOrderForCustomer(session.mobile, req.body || {});
        const id = typeof order.id === 'string' ? parseInt(order.id, 10) : Number(order.id);
        return res.status(200).json({
            ok: true,
            orderId: id,
        ...
        const code =
            err.statusCode && err.statusCode >= 400 && err.statusCode < 600 ? err.statusCode : 500;
        if (code >= 500) console.error('grocery order:', err.message);
        return res.status(code).json({ error: err.message || 'Could not place order.' });
    }
}
...
function sendGroceryAdminError(res, err, fallback) {
...
async function handleAdminGroceryCategories(req, res, method, categoryId) {
    const venue = await requireAdminApi(req, res);
    if (!venue) return;
    try {
        await ensureSchema();
        if (method === 'GET') {
            const data = await listGroceryCategoriesForAdmin(venue, groceryTargetVenueId(req));
            res.setHeader('Cache-Control', 'private, no-store, no-cache, must-revalidate');
            return res.status(200).json({ ok: true, ...data });
        }
        if (method === 'POST') {
            const category = await upsertGroceryCategory(venue, groceryTargetVenueId(req), req.body || {});
            return res.status(200).json({ ok: true, category });
        }
        if (method === 'PATCH') {
            const category = await upsertGroceryCategory(venue, groceryTargetVenueId(req), {
                ...(req.body || {}),
                id: categoryId
            });
            return res.status(200).json({ ok: true, category });
        ...
        if (method === 'DELETE') {
            const result = await deleteGroceryCategory(venue, groceryTargetVenueId(req), categoryId);
            return res.status(200).json({ ok: true, ...result });
        }
        return jsonError(res, 405, 'Method not allowed.');
    } catch (err) {
        return sendGroceryAdminError(res, err, 'Could not manage category.');
    }
}

async function handleAdminGroceryProducts(req, res, method, productId) {
    const venue = await requireAdminApi(req, res);
    if (!venue) return;
    try {
        await ensureSchema();
        if (method === 'GET') {
            const data = await listGroceryProductsForAdmin(venue, groceryTargetVenueId(req));
            res.setHeader('Cache-Control', 'private, no-store, no-cache, must-revalidate');
            return res.status(200).json({ ok: true, ...data });
        }
        if (method === 'POST') {
            const product = await upsertGroceryProduct(venue, groceryTargetVenueId(req), req.body || {});
            return res.status(200).json({ ok: true, product });
        }
        if (method === 'PATCH') {
            const product = await upsertGroceryProduct(venue, groceryTargetVenueId(req), {
                ...(req.body || {}),
                id: productId
            });
            return res.status(200).json({ ok: true, product });
        ...
        if (method === 'DELETE') {
            const result = await deleteGroceryProduct(venue, groceryTargetVenueId(req), productId);
            return res.status(200).json({ ok: true, ...result });
        }
        return jsonError(res, 405, 'Method not allowed.');
    } catch (err) {
        return sendGroceryAdminError(res, err, 'Could not manage product.');
    }
}

async function handlePostAdminGroceryStock(req, res, productId) {
    const venue = await requireAdminApi(req, res);
    if (!venue) return;
    try {
        await ensureSchema();
        const product = await adjustGroceryStock(venue, groceryTargetVenueId(req), productId, req.body || {});
        return res.status(200).json({ ok: true, product });
    } catch (err) {
        return sendGroceryAdminError(res, err, 'Could not adjust stock.');
    }
}

async function handleGetAdminGroceryLowStock(req, res) {
    const venue = await requireAdminApi(req, res);
    if (!venue) return;
    try {
        await ensureSchema();
        const data = await getLowStockProducts(venue, groceryTargetVenueId(req));
        res.setHeader('Cache-Control', 'private, no-store, no-cache, must-revalidate');
        return res.status(200).json({ ok: true, ...data });
    } catch (err) {
        return sendGroceryAdminError(res, err, 'Could not load low-stock items.');
    }
}

async function handleGetAdminDeliveryZones(req, res) {
    const venue = await requireAdminApi(req, res);
    if (!venue) return;
    try {
        await ensureSchema();
        const venueId = venue.isDefault ? null : venue.id;
        ...
        return jsonError(res, 500, 'Could not load delivery zones.');
    }
}

async function handlePostAdminDeliveryZones(req, res) {
    const venue = await requireAdminApi(req, res);
    if (!venue) return;
    try {
        await ensureSchema();
        ...
        return res.status(200).json({ ok: true, zone });
    } catch (err) {
        const code = err.statusCode && err.statusCode >= 400 && err.statusCode < 600 ? err.statusCode : 500;
...
async function handleDeleteAdminDeliveryZone(req, res, zoneId) {
    const venue = await requireAdminApi(req, res);
    if (!venue) return;
    try {
        await ensureSchema();
...
async function handleVercelApi(req, res) {
    ...
    if (method === 'POST' && pathname === '/api/admin/login') {
        return handlePostAdminLogin(req, res);
    }
    ...
    if (method === 'GET' && pathname === '/api/admin/all-orders-summary') {
        return handleGetAllOrdersSummary(req, res);
    }
    ...
    if (method === 'GET' && pathname === '/api/grocery') {
        return handleGetGrocery(req, res);
    }
    if (method === 'POST' && pathname === '/api/grocery/order') {
        return handlePostGroceryOrder(req, res);
    }
    if (pathname === '/api/admin/grocery/categories') {
        return handleAdminGroceryCategories(req, res, method, null);
    }
    if (pathname === '/api/admin/grocery/products') {
        return handleAdminGroceryProducts(req, res, method, null);
    }
    if (method === 'GET' && pathname === '/api/admin/grocery/low-stock') {
        return handleGetAdminGroceryLowStock(req, res);
    }
    ...
    if (method === 'GET' && pathname === '/api/admin/delivery-zones') {
        return handleGetAdminDeliveryZones(req, res);
    }
    if (method === 'POST' && pathname === '/api/admin/delivery-zones') {
        return handlePostAdminDeliveryZones(req, res);
    }
    ...
    if (method === 'DELETE' && adminVenuePatchMatch) {
        const venueId = parseInt(adminVenuePatchMatch[1], 10);
        if (!Number.isFinite(venueId)) return jsonError(res, 400, 'Invalid hotel id.');
        return handleDeleteAdminVenue(req, res, venueId);
    }

    const groceryStockMatch = pathname.match(/^\/api\/admin\/grocery\/products\/(\d+)\/stock$/);
    if (method === 'POST' && groceryStockMatch) {
        const productId = parseInt(groceryStockMatch[1], 10);
        if (!Number.isFinite(productId)) return jsonError(res, 400, 'Invalid product id.');
        return handlePostAdminGroceryStock(req, res, productId);
    }

    const groceryCategoryMatch = pathname.match(/^\/api\/admin\/grocery\/categories\/(\d+)$/);
    if ((method === 'PATCH' || method === 'DELETE') && groceryCategoryMatch) {
        const categoryId = parseInt(groceryCategoryMatch[1], 10);
        if (!Number.isFinite(categoryId)) return jsonError(res, 400, 'Invalid category id.');
        return handleAdminGroceryCategories(req, res, method, categoryId);
    }

    const groceryProductMatch = pathname.match(/^\/api\/admin\/grocery\/products\/(\d+)$/);
    if ((method === 'PATCH' || method === 'DELETE') && groceryProductMatch) {
        const productId = parseInt(groceryProductMatch[1], 10);
        if (!Number.isFinite(productId)) return jsonError(res, 400, 'Invalid product id.');
        return handleAdminGroceryProducts(req, res, method, productId);
    }

    const deliveryZoneDeleteMatch = pathname.match(/^\/api\/admin\/delivery-zones\/(\d+)$/);
    if (method === 'DELETE' && deliveryZoneDeleteMatch) {
        const zoneId = parseInt(deliveryZoneDeleteMatch[1], 10);
        if (!Number.isFinite(zoneId)) return jsonError(res, 400, 'Invalid zone id.');
        return handleDeleteAdminDeliveryZone(req, res, zoneId);
    }
