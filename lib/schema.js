...
async function ensureSchema() {
    ...
    await query(
        `INSERT INTO venues (id, slug, name, city, address_line, contact_mobile, hours_text, table_count, parcel_count, walkin_mobile, is_default)
         VALUES (1, 'quickkartcafe', 'QuickKart''s Cafe', 'Kudachi', 'Opp. Railway Station, Near Bus Stop, Kudachi – 591311, Karnataka', '9845812238', '1 PM – 10 PM', 7, 5, '8888888888', TRUE)
    ...
    // Unique index for venue_id IS NULL (global / QuickKart Cafe)
    await query(
        `CREATE UNIQUE INDEX IF NOT EXISTS idx_delivery_zones_null_venue_city
         ON delivery_zones (city) WHERE venue_id IS NULL`
    );

    // Seed default delivery zones for QuickKart Cafe (venue_id = NULL means main venue)
    await query(
