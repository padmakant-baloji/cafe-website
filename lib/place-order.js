...
const { assertOrderingWindowOpen } = require('./order-window');
...
async function placeOrderForCustomer(customerMobile, body) {
    ...
    if (cityLower === 'kudachi') {
        if (subtotal > 0 && subtotal < MIN_KUDACHI_FREE_DELIVERY_VALUE) {
            deliveryFee = KUDACHI_DELIVERY_FEE;
        }
    } else if (cityLower && subtotal >= MIN_NON_KUDACHI_ORDER_VALUE && subtotal < MIN_FREE_DELIVERY_ORDER_VALUE) {
        deliveryFee = NON_KUDACHI_DELIVERY_FEE;
    }
