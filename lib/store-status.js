...
async function getPublicStorefrontStatus() {
    ...
    const mainVenueStatus = buildEffectiveVenueStatus(mainVenue, mainStatus.acceptingOrders);
    const partners = await listPartnerVenueStatuses();
    const openPartners = partners.filter((venue) => venue.acceptingOrders);
    const venues = [
        { ...mainVenueStatus, isMain: true },
        ...partners.map((venue) => ({ ...venue, isMain: false }))
    ];

    if (mainVenueStatus.acceptingOrders) {
        return {
            acceptingOrders: true,
            activeVenueId: mainVenue.id,
            activeVenueName: mainVenue.name,
            mainVenueOpen: true,
            isFallbackVenue: false,
            fallbackMessage: null,
            reason: null,
    ...
    if (openPartners.length > 0) {
        const pick = openPartners[Math.floor(Math.random() * openPartners.length)];
        return {
            acceptingOrders: true,
            activeVenueId: pick.id,
            activeVenueName: pick.name,
            mainVenueOpen: false,
            isFallbackVenue: true,
            fallbackMessage: `${mainVenue.name} is closed right now. You can order from ${pick.name}.`,
            reason: null,
            notice: null,
            venues,
            updatedAt: mainStatus.updatedAt
    ...
    const showAdminNotice = !mainStatus.acceptingOrders && !anyVenueWithinHours;
    ...
    return {
        ...
        isFallbackVenue: false,
        fallbackMessage: null,
        reason: showAdminNotice ? mainStatus.reason : null,
        notice: showAdminNotice ? mainStatus.notice : null,
        venues,
        updatedAt: mainStatus.updatedAt
    };
}
