...
/** Loaded only when menu.json fails (keeps initial JS small). */
let cachedFallbackMenu = null;

async function loadFallbackMenuData() {
    if (cachedFallbackMenu) return cachedFallbackMenu;
    try {
        const res = await fetch('fallback-menu.json', { cache: 'force-cache' });
        if (res.ok) {
            const data = await res.json();
            if (data && Array.isArray(data.categories) && data.categories.length) {
                cachedFallbackMenu = data;
                return cachedFallbackMenu;
            }
        }
    } catch {
        /* ignore */
    }
    cachedFallbackMenu = { categories: [] };
    return cachedFallbackMenu;
}
...
async function loadMenu() {
    ...
    try {
        ...
        console.warn('Could not load menu.json, using fallback-menu.json:', error);
        menuData = await loadFallbackMenuData();
        renderMenu();
    } finally {
